...
class AdminSuggestionBanner extends Component
{
    use WithPagination;
    ...
    public function status($id)
    {
        ...
        $banner = SuggestionProducts::findOrFail($id);
        if ($banner->status == 0) {
            $banner->status = 1;
        } else {
            $banner->status = 0;
    ...
    public function deleteModel()
    {
        try {
            ...
            $model = SuggestionProducts::findOrFail($this->banner_id);
    ...
    {
        return view('livewire.admin.banner2.admin-suggestion-banner')
            ->extends('admin_end.include.master_dash')
            ->section('dash_main_content')
            ->with(['banners' => SuggestionProducts::paginate(10) ]);
    }
}
